...
<div class="min-h-screen p-0 bg-gray-100 sm:p-12">
  <div class="max-w-md px-6 py-12 mx-auto bg-white border-0 shadow-lg sm:rounded-3xl">
    <h1 class="mb-8 text-2xl font-bold">Modify a team</h1>
    <form id="form" action={{route('team.update', $team->id)}} method="POST">
        @csrf
        @method("PUT")
      <div class="relative z-0 w-full mb-5">
        ...
        />
        <label for="club" class="absolute text-gray-500 duration-300 top-3 -z-1 origin-0">Club</label>
      ...
      <div class="relative z-0 w-full mb-5">
        <input
          ...
          class="block w-full px-0 pt-3 pb-2 mt-0 bg-transparent border-0 border-b-2 border-gray-200 appearance-none focus:outline-none focus:ring-0 focus:border-black"
        />
        <label for="city" class="absolute text-gray-500 duration-300 top-3 -z-1 origin-0">City</label>
  ...
  <div class="relative z-0 w-full mb-5">
        <select
          name="continent_id"
          value="{{$team->continents->continent}}"
        ...
        <option {{$continent->continent == $team->continents->continent ? "selected" : " "}} value="{{ $continent->id}}">{{ $continent->continent }}</option>
          @endforeach
        </select>
      ...
      <div class="relative z-0 w-full mb-5">
        ...
        value="{{$team->maxplayers}}"
          ...
          class="block w-full px-0 pt-3 pb-2 mt-0 bg-transparent border-0 border-b-2 border-gray-200 appearance-none focus:outline-none focus:ring-0 focus:border-black"
        />
        <label for="country" class="absolute text-gray-500 duration-300 top-3 -z-1 origin-0">Max Players</label>
        <span class="hidden text-sm text-red-600" id="error">Max is required</span>
      </div>

      <p>Nombre de joueurs : {{count($team->players)}} / {{$team->maxplayers}}</p>
